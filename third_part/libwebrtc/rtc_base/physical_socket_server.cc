...
// 检查是否支持 SIOCGSTAMP（Linux 特性，iOS 不支持）
...
#else
// iOS、macOS、Android 等不支持 SIOCGSTAMP
int64_t GetSocketRecvTimestamp(int socket) {
  return -1;
}
#endif
  ...
  switch (opt) {
      case OPT_DONTFRAGMENT:
            ...
            *sopt = IP_DONTFRAGMENT;
            ...
            return -1;
      #elif defined(WEBRTC_POSIX)
            // 只有 Linux 支持 IP_MTU_DISCOVER
            // iOS 和 Android 可能不支持或不完全支持
      #if defined(WEBRTC_LINUX) && !defined(WEBRTC_ANDROID)
            *slevel = IPPROTO_IP;
            *sopt = IP_MTU_DISCOVER;
            break;
      #else
            RTC_LOG(LS_WARNING) << "Socket::OPT_DONTFRAGMENT not supported on this POSIX platform.";
            return -1;
      #endif
      #endif
    case OPT_RCVBUF:
