...
Optional<signed_type> ParseSigned(const char* str, int base);
Optional<unsigned_type> ParseUnsigned(const char* str, int base);

template <typename T>
Optional<T> ParseFloatingPoint(const char* str);
}  // namespace string_to_number_internal
