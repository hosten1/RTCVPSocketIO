                                        ...
                                        int proto, uint32_t timeout )
{
    ...
    printf("mbedtls_net_connect_timeout ret1:%d\n",ret);
    printf("mbedtls_net_connect_timeout addr_list.ai_family:%d\n",addr_list->ai_family);
    printf("mbedtls_net_connect_timeout addr_list.ai_socktype:%d\n",addr_list->ai_socktype);
    printf("mbedtls_net_connect_timeout addr_list.ai_protocol:%d\n",addr_list->ai_protocol);
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        printf("mbedtls_net_connect_timeout cur.ai_family:%d\n",cur->ai_family);
        printf("mbedtls_net_connect_timeout cur.ai_socktype:%d\n",cur->ai_socktype);
        printf("mbedtls_net_connect_timeout cur.ai_protocol:%d\n",cur->ai_protocol);
        ...
        printf("mbedtls_net_connect_timeout connect ctx->fd:%d\n",ctx->fd);
        printf("mbedtls_net_connect_timeout connect cur->ai_addr.sa_family:%d\ncur->ai_addr.sa_data:%s\n",cur->ai_addr->sa_family,cur->ai_addr->sa_data);
        printf("mbedtls_net_connect_timeout connect cur->ai_addrlen:%d\n",cur->ai_addrlen);
        int connectRet = connect( ctx->fd, cur->ai_addr, cur->ai_addrlen );
        printf("mbedtls_net_connect_timeout connect connectRet:%d\n",connectRet);
        printf("mbedtls_net_connect_timeout connect errno:%d\n",errno);
        if(  connectRet == 0 )
        {
            printf("mbedtls_net_connect_timeout connect Success\n");
            ret = 0;
            break;
        }
        else if( errno == EINPROGRESS )
        {
            printf("mbedtls_net_connect_timeout connect errno = EINPROGRESS\n");
            int            fd = (int)ctx->fd;
            int            opt;
            socklen_t      slen;
            struct timeval tv;
            ...
            printf("mbedtls_net_connect_timeout connect errno = 0\n");
            int            fd = (int)ctx->fd;
            int            opt;
            socklen_t      slen;
            struct timeval tv;
