...
// ============================================================================
// 测试嵌套结构
// 测试目标: 验证包含二进制数据的复杂嵌套JSON结构的拆分和合并
// 测试场景: 
// 1. 创建包含多个二进制数据的嵌套JSON对象
// 2. 使用PacketSplitter异步拆分为文本和二进制部分
// 3. 验证拆分结果的正确性
// 4. 使用PacketSplitter异步合并回原始结构
// 5. 验证合并后的数据与原始数据完全一致
// ============================================================================
 ...
 * 测试嵌套结构中的二进制数据处理
 * Socket.IO二进制数据处理机制: 
 * - 发送端: 将二进制数据替换为占位符 {"_placeholder":true, "num":0}
 * - 接收端: 根据占位符索引将二进制数据重新插入到JSON结构中
 * - 支持嵌套结构中的多个二进制数据
 */
...
// 测试目标: 验证PacketSender和PacketReceiver的完整工作流程
// 测试场景: 
// 1. 创建包含二进制数据的复杂事件
// 2. 使用PacketSender将事件拆分为文本和二进制部分
// 3. 使用PacketReceiver将文本和二进制部分合并
// 4. 验证合并后的数据与原始数据一致
// 5. 测试不同Socket.IO版本的兼容性
// ============================================================================

/**
 * 测试PacketSender和PacketReceiver的完整工作流程
 * Socket.IO事件发送和接收机制: 
 * - PacketSender: 将OC对象转换为JSON格式，处理二进制数据
 * - PacketReceiver: 解析JSON包，重构OC对象
 * - 支持多种数据类型: 字符串、数字、布尔值、二进制数据
 */
...
// ============================================================================
// 测试版本兼容性
// 测试目标: 验证不同Socket.IO版本间的兼容性
// 测试场景: 
// 1. 测试Socket.IO v2版本的数据包格式
// 2. 测试Socket.IO v3版本的数据包格式
// 3. 测试单个二进制数据的处理
// 4. 测试多个二进制数据的处理
// 5. 测试PacketHelper工具类的功能
// ============================================================================

/**
 * 测试不同Socket.IO版本间的兼容性
 * Socket.IO版本差异: 
